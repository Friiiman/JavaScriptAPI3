...
function likeBtnRise() {
    const likeBtnEl = document.querySelector('.like-btn');
    const likeCountEl = document.querySelector('.like-count');

    const getData = JSON.parse(localStorage.getItem(localStorageKey));
    const closestImageElId = likeBtnEl.closest('.image').getAttribute('id');
    const getCurrentData = getData.filter((element) => element.id === closestImageElId);
    const getCurrentDataIndex = getData.findIndex(obj => obj.id === closestImageElId);
...
function saveToHistory(currentImage) {
    ...
    const setCurrentData = JSON.stringify(currentData);
    const allHistory = localStorage.getItem(historyLocalStorageKey);
    if (!allHistory) {
        localStorage.setItem(historyLocalStorageKey, setCurrentData);
    }

    const getData = JSON.parse(localStorage.getItem(historyLocalStorageKey));
    const getCurrentData = getData.filter((element) => element.id === currentImage.id);
    if (getCurrentData.length === 0) {
        getData.push(...currentData);
        localStorage.setItem(historyLocalStorageKey, JSON.stringify(getData));
    }
